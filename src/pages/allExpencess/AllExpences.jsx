...
const schema = Joi.object({
  ...
  name: Joi.string().required().label("Expence Name"),
...
const AllExpences = () => {
  ...
  const filterdata = () => {
    dispatch(getMonthExpences(filter));
  };
  const clearFilter = () => {
    let searchObj = {
      month_id: "",
    ...
    dispatch(getMonthExpences(searchObj));
    clearFilterForm();
  };
  ...
  const submit = async () => {
    ...
    let data = await dispatch(addMonthExpence(form)); // save new expences data
    if (data) {
      handleCancelAddModal();
      await message.success({
        content: "Expences Added Successfully",
        style: {
          marginTop: "10vh",
        },
      });
  ...
  return (
    <div className="all-expences">
      <div className="all-expences-filter">
        <AddBoxIcon className="all-expences-addIcon" onClick={handleAddModal} />
